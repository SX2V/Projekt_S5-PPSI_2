...
const handleSubmit = async () => {
  ...
  try {
    if (isEditing.value && form.value.id) {
      ...
      toast.success(t('admin.sportUpdated'));
    } else {
    ...
    toast.error(t('admin.saveSportFailed'));
  }
};

const deleteSport = async (id: string) => {
  if (!confirm(t('admin.confirmDeleteSport'))) return;
  
  try {
    await apiClient.delete(`/Sports/${id}`);
    toast.success(t('admin.sportDeleted'));
    fetchSports();
  } catch (error) {
    console.error('Failed to delete sport', error);
    toast.error(t('admin.deleteSportFailed'));
  }
};
...
<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('admin.sportsManagement') }}</h2>
      <button 
        @click="openAddModal"
    ...
    <BaseModal 
      :is-open="isModalOpen" 
      :title="isEditing ? t('admin.editSport') : t('profile.addNewSport')"
    ...
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.typicalDistanceKm') }}</label>
        ...
        <div class="mt-5 sm:mt-6 flex justify-end space-x-3">
          ...
          >
            {{ isEditing ? t('admin.saveChanges') : t('profile.addSport') }}
          </button>
        </div>
      </form>
    </BaseModal>
